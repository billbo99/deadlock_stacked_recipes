...
--- Get the localised name of the item with the given name.
	...
	elseif item.place_result then return Locale.get_entity_locale(item.place_result)
	elseif item.placed_as_equipment_result then return Locale.get_equipment_locale(item.placed_as_equipment_result)
...
--- Get a pair of `name, type` for the main product of the recipe.
	...
	elseif recipe.main_product ~= nil then return Locale.find_product(recipe, recipe.main_product)
	elseif recipe.results then
		if table_size(recipe.results) == 1 then return Locale.parse_product(recipe.results[1])
		else return nil, nil; end
	else return Locale.parse_product(recipe.result); end
end
...
--- Get the localised name for the given recipe.
	...
	if recipe.normal ~= nil and recipe.expensive ~= nil then
		...
		if normal_product ~= nil and normal_product == expensive_product and normal_type == expensive_type then
			if normal_type == 'item' then return Locale.get_item_locale(normal_product)
			else return Locale.get_fluid_locale(normal_product); end
	...
	if main_product ~= nil then
		if main_type == 'item' then return Locale.get_item_locale(main_product)
		else return Locale.get_fluid_locale(main_product); end
	else return {'recipe-name.'..recipe.name}; end
end
